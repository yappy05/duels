...
export class TournamentService {
  ...
  public async create(dto: CreateTournamentRequestDto) {
    ...
    if (countParticipants <= 0 || (countParticipants & (countParticipants - 1)) !== 0) {
      throw new RpcException({
        code: 4,
        message: 'количество участников должно быть степенью двойки (2, 4, 8, 16, 32, ...)',
      });
    }
  ...
  public async getResults(dto: TournamentInfo) {
    ...
    let maxPosition = Math.pow(maxRounds, 2);
    if (maxPosition === 1) maxPosition = 2
  ...
  public async generateNextRound(dto: AddRoundRequestDto) {
    ...
    if (!tournament) throw new RpcException({ code: 5, message: 'турнир не найден' });
    if (tournament.status === TournamentStatus.FINISHED) throw new RpcException(({
      code: 6,
      message: 'турнир уже закончен',
    }));
    const rounds = await this.getRounds(dto);
    for (const round of rounds) {
      if (round?.id && round?.status === RoundStatus.ACTIVE) throw new RpcException({
        code: 6,
        message: 'нельзя начать рануд, предыдущий еще не закончился',
      });
    }
